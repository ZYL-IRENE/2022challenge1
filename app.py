...
# node_path = 'D:\\UMJI\\2021 Spring\\PRP\\China_vis\\2022\\code\\Node.csv'
# link_path = 'D:\\UMJI\\2021 Spring\\PRP\\China_vis\\2022\\code\\Link.csv'
#
# pd_link = pd.read_csv(link_path)
# pd_node = pd.read_csv(node_path)
...
def getNodeCsv(idx, is_contain_empty_industry, cert_n, ratio):
    idx = 1
    filepath = 'D:\\UMJI\\2021 Spring\\PRP\\China_vis\\2022\\code\\data_process\\output\\'+str(idx)+'_'+\
               str(is_contain_empty_industry)+'_'+str(cert_n)+'_'+str(ratio)+'_nodes.csv'
...
def getLinkCsv(idx, is_contain_empty_industry, cert_n, ratio):
    idx = 1
    filepath = 'D:\\UMJI\\2021 Spring\\PRP\\China_vis\\2022\\code\\data_process\\output\\'+str(idx)+'_'+\
               str(is_contain_empty_industry)+'_'+str(cert_n)+'_'+str(ratio)+'_links.csv'
    data = pd.read_csv(filepath)
    return data


def getCenterNodesCsv(idx, is_contain_empty_industry, cert_n, ratio):
    idx = 1
    filepath = 'D:\\UMJI\\2021 Spring\\PRP\\China_vis\\2022\\code\\data_process\\output\\' + str(idx) + '_' + \
               str(is_contain_empty_industry) + '_' + str(cert_n) + '_' + str(ratio) + '_centerNodes.csv'
    data = pd.read_csv(filepath)
    return data
...
#
#
# def get_center(fig, ratio: float = 0.25):
#     centers = fig.vs.select(_degree_ge=fig.maxdegree() * ratio)
#     res_centers = []
#     res_centers_idx = []
#     res_centers_id = []
...
#
#
# def operate(test_num: int, is_contain_empty_industry: bool = True, cert_n: int = 100, ratio: float = 0.25):
...
#
#     # ig.plot(fig)
#     centerNodes, centerNodes_idx, centerNodes_id = get_center(fig, ratio)
...
#
#     return df_nodes, df_links, df_centerNodes, df_keylines
...
@app.route('/dataByindustry', methods=['GET'])
def getDataByIndustry():
    get_data = request.args.to_dict()
    is_contain_empty_industry = get_data.get('is_contain_empty_industry')
    cert_n = get_data.get('cert_n')
    ratio = get_data.get('ratio')

    # nodes = getNodeCsv(1, is_contain_empty_industry, cert_n, ratio)
    # nodesData = []
    # for index, row in nodes.iterrows():
    #     temp = {'id': row['id'], 'name': row['name'], 'type': row['type'], 'industry': row['industry']}
    #     nodesData.append(temp)
    #
    # links = getLinkCsv(1, is_contain_empty_industry, cert_n, ratio)
    # linksData = []
    # for index, row in links.iterrows():
    #     temp = {'source': row['source'], 'target': row['target']}
    #     linksData.append(temp)
    #
    # dic = {'nodes': nodesData, 'links': linksData}
    # return jsonify(dic)
    return str(is_contain_empty_industry)+str(cert_n)+str(ratio)


@app.route('/dataBycert', methods=['GET'])
def getDataBycert():
    get_data = request.args.to_dict()
    is_contain_empty_industry = get_data.get('is_contain_empty_industry')
    cert_n = get_data.get('cert_n')
...
@app.route('/dataByratio', methods=['GET'])
def getDataByratio():
    get_data = request.args.to_dict()
    is_contain_empty_industry = get_data.get('is_contain_empty_industry')
    cert_n = get_data.get('cert_n')
...
# def getCenterNodes():
#     is_contain_empty_industry = request.form.get('is_contain_empty_industry')
#     cert_n = request.form.get('cert_n')
#     ratio = request.form.get('center_ratio')
#     centerNodes = getCenterNodesCsv(1, is_contain_empty_industry, cert_n, ratio)
#     centerNodesData = []
#     for index, row in centerNodes.iterrows():
#         temp = {}
