...
def get_jump(node: str) -> set:
    ...
    and need to judge if search target or source
...
def get_all_jump(node: str) -> set:
    """
    Get the neighbor nodes of node within one jump without judge
...
def get_first_jump(node: str):
    """
    For the first link between node and the neighbor node in one-jump,
    and need to judge by the relation of link
...
def get_node_by_bfs(node: str) -> Tuple[set, set, set]:
    ...
    set_b_2 = set([])
    set_c = set([])
    dict_a = get_first_jump(test_node)
    print(dict_a)
    for A in dict_a:
        print('begin {}'.format(A))
        if dict_a[A][2] == 's' or dict_a[A][2] == 'r' or dict_a[A][2] == 'd':
            for i in get_all_jump(A):
                set_b_2.add(i)
        elif dict_a[A] == 'r_cert':
            for i in get_all_jump(A):
                set_b_1.add(i)
        elif dict_a[A][2] == 'w':
            for i in get_all_jump(A):
...
res_set = set([])
for temp_set in list_set:
    res_set = res_set | temp_set

time_end = time.time()
time_c = time_end - time_start

print(len(res_set))  # the number of the nodes
idx = 0
for key in edges_all:
    for target in edges_all[key]:
        idx += 1
print(idx)  # the number of the links

print('time cost', time_c, 's')  # time
